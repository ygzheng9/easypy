...
def pagesURL(image_type, position):
    '''
    ...
    pages_num_list = images_information[-1].get('href').replace('.',
                                                                '_').split('_')
    # 当前图片类型共有多少页
...
def save_images(atlas_url):
    ...
    for url in atlas_url:
        ...
        # 当前图集的编号，如图集链接 http://www.mm131.com/qingchun/3683.html，编号为 3683，用于拼接当前图集的图片地址
        pic_number = url.replace('.', '/').split('/')[-2]
        # 创建列表，用于保存每一张图片的链接
        images_url = []
        for number in range(1, images_number + 1):
            images_url.append('http://img1.mm131.me/pic/' + pic_number + '/' +
                              str(number) + '.jpg')
        ...
        headers = {
            ...
            'referer':
            "http://www.mm131.com/xinggan/530.html"
        }
        # 开始下载提示，等待2秒后开始下载
...
pagesURL = pagesURL('qingchun', '1')

# 获取页面的所有图集链接
atlas_url = atlas(pagesURL)
