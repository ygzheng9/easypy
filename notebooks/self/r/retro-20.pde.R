...
Diffusion <- function(t, Y, parms) {
  ...
  list(
    dY = tran$dC, flux.up = tran$flux.up,
...
image(out,
  grid = x, mfrow = NULL, ylab = "Distance, x",
  main = "Y"
)
...
matplot.1D(out,
  which = "u", subset = time %in% outtime,
...
image(out,
  grid = list(x, y), main = "elliptic Laplace",
  add.contour = TRUE
)
...
times <- seq(0, 20, 0.01)
out1 <- ode.1D(
  y = yini, func = adv.func, times = times,
...
#############################################################
# 5. The Busselator in One Dimension
N <- 50
...
brusselator1D <- function(t, y, parms) {
  ...
  dX1 <- 1 + X1^2 * X2 - 4 * X1 +
    tran.1D(
      C = X1, C.up = 1, C.down = 1,
      D = 0.02, dx = Grid
    )$dC
  dX2 <- 3 * X1 - X1^2 * X2 +
    tran.1D(
      C = X2, C.up = 3, C.down = 3,
      D = 0.02, dx = Grid
    )$dC
  list(c(dX1, dX2))
}
...
brusselator2D <- function(t, y, parms) {
  ...
  dX1 <- 1 + X1^2 * X2 - 4 * X1 +
    tran.2D(
      C = X1, D.x = D_X1, D.y = D_X1,
      dx = Gridx, dy = Gridy
    )$dC
  dX2 <- 3 * X1 - X1^2 * X2 +
    tran.2D(
      C = X2, D.x = D_X2, D.y = D_X2,
      dx = Gridx, dy = Gridy
    )$dC
  list(c(dX1, dX2))
}
...
print(system.time(
  out <- ode.2D(
    y = yini, parms = NULL, func = brusselator2D,
...
#####################################################
# 7. The Laplace Equation in Polar Coordinates
Nr <- 100
Np <- 100
r <- seq(2, 4, len = Nr+1)
...
#####################################################
# 8. The Time-dependent 2-D Sine-Gordon Equation
Nx <- 80
Ny <- 80
...
##################################################################
# 9. The Nonlinear Schrodinger Equation
alf <- 0.5
gam <- 1
