## 逻辑回归；
## odds = win / loss 也即： p / (1 - p), 不是 probability；
## odds 是 y 轴；取值 [0, 正无穷], 其中，[0, 1] 是输了，[1, 正无穷] 是赢了；两个区间完全不成比例；
## 取 log 后，投影到 负无穷 到 正无穷；0 对应胜负对半，纵轴均匀分布了；
## x 轴还是原先的物理量；
## 依然可以做一条直线，但是 最小二乘法 不能用，因为 y 值都在正负无穷远处，但是可以求得在 x 处，直线上对应的点的 y 值；
## 而 y = log(odds), odds = p / (1 - p);进而可求出 p；
## 样本有很多点，也就能求出很多 p；
## 利用 极大似然 的方法，似然函数是 p 的连乘，求出最可能下的直线参数，就是 logistic 回归；
...
data$ca <-
    as.factor(data$ca)  # ...then convert the integers to factor levels
...
## a sample has sex=F.
# odds 是 发生数 / 不发生数，也即： unhealth / health
# 斜率，表示 自变量增加一个，y 增加多少；这里，自变量只有两个值，0 female，1 male，按字母排序的；
# y 对于 LR 来说，就是 log(odds) 也即：male 的 log(odds) 比 female 的 log(odds) 大 1.27
...
predicted.data <- predicted.data[order(predicted.data$probability.of.hd, decreasing =
                                           FALSE), ]
